/**
 * Single source of truth for marina workspace navigation.
 *
 *   MARINA_NAV_GROUPS — used by MarinaRail (desktop + tablet)
 *   MOBILE_TABS      — used by MarinaTabBar (mobile)
 *
 * The `counter` field, when set, names the key on the useMarinaCounters() result
 * whose value should render as the unread badge for that item.
 */
