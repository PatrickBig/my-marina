...
- **Customer JWT claims:** On login, `CustomerAccountId` is looked up from `CustomerAccountMembers` and embedded as `customer_account_id` claim. All portal query handlers filter by both `TenantId` (EF global filter) and `CustomerAccountId` (explicit WHERE) — never expose cross-customer data.
- **Customer invitation:** `POST /customers/{id}/invite` creates an `ApplicationUser` with `UserRole.Customer` + a `CustomerAccountMember` (Owner role). Returns a generated temporary password. Duplicate email → 409 Conflict.
