...
Phase 1 complete. Solution scaffolded and building cleanly (0 errors, 0 warnings).  
EF Core migrations not yet created — run `dotnet ef migrations add InitialCreate --project src/MyMarina.Infrastructure --startup-project src/MyMarina.Api` after Postgres is running.
...
- **Multi-tenancy:** Shared DB, shared schema, EF Core global query filters on `TenantId`. Two-level scoping: `ITenantContext` (cross-tenant) + `IMarinaContext` (per-marina staff). Both implemented by `HttpTenantContext` in Infrastructure — resolved from JWT claims.
...
src/
  ...
  MyMarina.Infrastructure/  # EF Core, Postgres, Identity, Hangfire+Redis, multi-tenancy
    ...
    MultiTenancy/           # HttpTenantContext (implements ITenantContext + IMarinaContext)
