...
- **EF global query filter rule:** Every entity that has a `HasQueryFilter` must propagate a matching filter to all child entities that have a required FK pointing at it. Failing to do so produces EF warning 10622 and can silently return incomplete data. Pattern: if `Parent` filters on `p => p.Marina.IsSetupComplete`, then every child with a required `ParentId` FK must also add `HasQueryFilter(c => c.Parent.Marina.IsSetupComplete)`. This applies transitively — grandchildren must chain through both intermediate navigations. The affected configurations live in `src/MyMarina.Infrastructure/Persistence/Configurations/`. When adding a new entity that has a required FK to a filtered parent, add the matching filter in the same PR.
