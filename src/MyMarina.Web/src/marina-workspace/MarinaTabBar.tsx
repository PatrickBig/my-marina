...
export function MarinaTabBar({ marinaId }: { marinaId: string }) {
  ...
  return (
    ...
    >
      {MOBILE_TABS.map((tab) => {
        const Icon = tab.icon;
        const isActive = tab.id === 'more' ? inMore : activeId === tab.id;
        const count = tab.counter ? counters?.[tab.counter] : undefined;

        if (tab.id === 'more') {
          return (
            <Sheet key="more" open={moreOpen} onOpenChange={setMoreOpen}>
              <SheetTrigger asChild>
                <button type="button" className={tabButtonClass(isActive)}>
