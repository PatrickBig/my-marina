...
internal static class SlipAvailabilityFilter
{
    ...
    // Lightweight projection used by the marina rollup handler — avoids loading full Slip entities.
    internal sealed record RollupSummary(Guid MarinaId, decimal Price, string RateKind, bool InstantBook);

    internal static async Task<List<RollupSummary>> GetRollupSummariesAsync(
        AppDbContext db, IQueryable<Slip> baseSlipQuery, Filters f, CancellationToken ct)
    {
        SlipType slipTypeFilter = default;
        bool hasSlipTypeFilter = !string.IsNullOrWhiteSpace(f.SlipType)
            && Enum.TryParse(f.SlipType, ignoreCase: true, out slipTypeFilter);
        ...
        return f.IsLease
            ? await GetLeaseRollupSummariesAsync(db, slipQ, f, today, ct)
            : await GetTransientRollupSummariesAsync(db, slipQ, f, today, ct);
    }

    private static async Task<List<RollupSummary>> GetTransientRollupSummariesAsync(
        AppDbContext db, IQueryable<Slip> slipQ, Filters f, DateOnly today, CancellationToken ct)
    {
        var arrivesAt     = (f.ArrivesAt ?? today).ToDateTime(TimeOnly.MinValue, DateTimeKind.Utc);
        var departsAt     = (f.DepartsAt ?? f.ArrivesAt?.AddDays(1) ?? today.AddDays(1))
                                .ToDateTime(TimeOnly.MinValue, DateTimeKind.Utc);
        ...
        var results      = new List<RollupSummary>();
        var pathASlipIds = new HashSet<Guid>();

        if (windowSlipIds.Count > 0)
        {
            // Project only (Id, MarinaId) — avoid loading full Slip entities
            var windowSlipCores = await slipQ
                .Where(s => windowSlipIds.Contains(s.Id))
                .Select(s => new { s.Id, s.MarinaId })
                .ToListAsync(ct);

            if (windowSlipCores.Count > 0)
            {
                var slipIdList    = windowSlipCores.Select(x => x.Id).ToList();
                var marinaBySlipId = windowSlipCores.ToDictionary(x => x.Id, x => x.MarinaId);
                ...
                var windows = await db.AvailabilityWindows
                    .Where(w => slipIdList.Contains(w.SlipId)
                             ...
                             && w.StartsAt <= arrivesAt
                             && w.EndsAt   >= departsAt)
                    .Select(w => new { w.SlipId, Price = w.BasePricePerNight, w.RateKind, w.InstantBook })
                    .ToListAsync(ct);

                foreach (var w in windows.GroupBy(w => w.SlipId)
                                         .Select(g => g.OrderBy(w => w.Price).First())
                                         .Where(w => marinaBySlipId.ContainsKey(w.SlipId)))
                {
                    results.Add(new RollupSummary(marinaBySlipId[w.SlipId], w.Price, w.RateKind.ToString(), w.InstantBook));
                    pathASlipIds.Add(w.SlipId);
                }
            }
        }

        // Path B: direct default transient rate — project only aggregation fields
        var directSummaries = await slipQ
            .Where(s => s.DefaultTransientRateKind != null
                     && !pathASlipIds.Contains(s.Id)
                     && !db.SlipAssignments.Any(a =>
                            a.SlipId == s.Id &&
                            a.StartDate <= departsDateOnly &&
                            (a.EndDate == null || a.EndDate >= arrivesDateOnly))
                     && !db.Reservations.Any(r =>
                            ...
                            r.Status != ReservationStatus.Declined &&
                            r.Status != ReservationStatus.Cancelled))
            .Select(s => new { s.MarinaId, Price = s.DefaultTransientBaseRate!.Value, RateKind = s.DefaultTransientRateKind })
            .ToListAsync(ct);

        results.AddRange(directSummaries.Select(x =>
            new RollupSummary(x.MarinaId, x.Price, x.RateKind!.Value.ToString(), true)));

        return results;
    }

    private static async Task<List<RollupSummary>> GetLeaseRollupSummariesAsync(
        AppDbContext db, IQueryable<Slip> slipQ, Filters f, DateOnly today, CancellationToken ct)
    {
        DateTime? desiredStart = f.ArrivesAt.HasValue
            ? f.ArrivesAt.Value.ToDateTime(TimeOnly.MinValue, DateTimeKind.Utc)
            ...
            if (windowSlipCores.Count > 0)
            {
                ...
                var windows = await db.AvailabilityWindows
                    .Where(w => slipIdList.Contains(w.SlipId)
                             ...
                             && w.ListingKind == ListingKind.Lease
                             && (f.LeaseTermFilter == null || w.LeaseTerm == f.LeaseTermFilter))
                    .Select(w => new { w.SlipId, Price = w.BasePricePerNight, w.RateKind })
                    .ToListAsync(ct);
                ...
                foreach (var w in windows.GroupBy(w => w.SlipId)
                                         ...
                                         .Where(w => marinaBySlipId.ContainsKey(w.SlipId)))
                {
                    results.Add(new RollupSummary(marinaBySlipId[w.SlipId], w.Price, w.RateKind.ToString(), false));
                    pathASlipIds.Add(w.SlipId);
                }
            }
        }
        ...
        var directSummaries = await slipQ
            .Where(s => s.DefaultLeaseRateKind != null
                     ...
                     && !db.SlipAssignments.Any(a => a.SlipId == s.Id && (a.EndDate == null || a.EndDate >= today))
                     && (f.LeaseTermFilter == null || s.DefaultLeaseTerm == f.LeaseTermFilter))
            .Select(s => new { s.MarinaId, Price = s.DefaultLeaseBaseRate!.Value, RateKind = s.DefaultLeaseRateKind })
            .ToListAsync(ct);

        results.AddRange(directSummaries.Select(x =>
            new RollupSummary(x.MarinaId, x.Price, x.RateKind!.Value.ToString(), false)));
