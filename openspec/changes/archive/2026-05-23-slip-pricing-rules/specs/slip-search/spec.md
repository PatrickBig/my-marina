...
The `priceMin` and `priceMax` filters SHALL only be applied when `listingKind` is also provided. The price source SHALL be the resolved price materialized on the slip: `Slip.ResolvedTransientBaseRate` when `listingKind=Transient`, `Slip.ResolvedLeaseBaseRate` when `listingKind=Lease`. The query SHALL NOT invoke the `PriceResolver` inline per row. A slip whose resolved rate for the requested listing kind is `NULL` SHALL be excluded entirely from the result. A slip passes the price filter if its resolved rate falls within the specified range. If `priceMin` or `priceMax` is supplied without `listingKind`, the server SHALL return a `400 Bad Request`.
...
- **AND** Sunset Point Marina has 24 active slips that fit the boat with `ResolvedTransientBaseRate` non-null
...
#### Scenario: Price range filter reads resolved rate

- **WHEN** a boater searches with `listingKind=Transient`, `priceMin=100`, `priceMax=200`
- **AND** Sunset Point has slips with `ResolvedTransientBaseRate` of `$150` and `$250`
- **THEN** Sunset Point IS included in the response (the $150 slip falls within $100–$200)
- **AND** `AvailableCount` reflects only the slips that match ALL filters including price
- **AND** the SQL filters on `Slip.ResolvedTransientBaseRate BETWEEN 100 AND 200`
- **AND** the `PriceResolver` is NOT invoked

#### Scenario: Slips with null resolved rate are excluded

- **WHEN** a boater searches with `listingKind=Lease`
- **AND** Harbor View Marina has slips with `ResolvedLeaseBaseRate = NULL` (no matching base rule)
- **THEN** those slips are excluded from `AvailableCount`
- **AND** if no other slips at the marina match, the marina is excluded entirely
...
- **AND** all of Harbor View Marina's slips have `ResolvedTransientBaseRate` above $50
- **THEN** Harbor View Marina is excluded from the response

#### Scenario: Price filter without listing kind returns 400

- **WHEN** a boater calls the endpoint with `priceMin=100` and no `listingKind`
